...
agent any
    ...
    stages {  
        ...
        stage ("Clone Repository") {
                steps {
                   git branch: 'master', url: 'https://github.com/RamanaKondaveeti/dotnetwebapp.git'
                }
            }  
        stage('Prep') {
            steps {
        ...
        stage ('Copy proj to backup') {
            steps {
                script {
                    ...
                    sh "cp -r ${project_folder} ${backup_folder}/${JOB_NAME}_${currentBuild.number}_${date +"%d-%m-%Y"}"
                }
            }
        }
